...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (post.username === req.body.username) {
      ...
      //   if (req.body.photo) {
      //     cloudinary.uploader.destroy(post.public_id, {invalidate: true}, function(error, result) {
...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    if (username) {
      ...
      posts = await Post.find(4); // ưu tiên lấy num_results_on_page do page có thể bằng 1 nên else if (page) được thực hiện
    }
